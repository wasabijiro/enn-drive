...
export default function Page() {
  ...
  const [userId, setUserId] = useState(1); // user_id ステートの追加
  ...
  }, [userId]); // userIdが変更された時にもインターバルを再設定

  // @ts-ignore
  const handleUserIdChange = (event) => {
    setUserId(event.target.value); // user_id の更新
  };
  ...
  const addLocation = async () => {
    try {
      ...
      const newLocation = await apiRequest("/api/location", {
        method: "POST",
        body: JSON.stringify({ user_id: userId, latitude, longitude }), // userId を使用
      });
  ...
  return (
    <div className="flex h-screen justify-center items-center relative">
      <input
        type="text"
        value={userId} // input の value を userId に設定
        onChange={handleUserIdChange} // input の変更をハンドル
